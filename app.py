...
def deep_research(brand):
    import concurrent.futures
    queries = [
        f"{brand} brand overview company history India",
        f"{brand} revenue funding investors valuation 2025",
        f"{brand} company registration MCA CIN directors",
        f"{brand} startup India DPIIT recognition",
        f"{brand} zauba tofler company data",
        f"{brand} inc42 yourStory funding rounds",
        f"{brand} instagram followers engagement 2025",
        f"{brand} facebook page followers likes",
        f"{brand} linkedin company followers",
        f"{brand} twitter X followers engagement",
        f"{brand} youtube channel subscribers views",
        f"{brand} meta ads facebook advertising library",
        f"{brand} google ads transparency advertising",
        f"{brand} semrush traffic keywords domain",
        f"{brand} target audience ICP demographics India",
        f"{brand} competitors analysis market share India",
        f"{brand} market size TAM India",
        f"{brand} SWOT analysis strengths weaknesses",
        f"{brand} site:amazon.in reviews rating",
        f"{brand} site:flipkart.com reviews rating",
        f"{brand} site:trustpilot.com reviews",
        f"{brand} google reviews rating feedback",
        f"{brand} site:reddit.com reviews discussion",
        f"{brand} site:quora.com questions answers",
        f"{brand} shopping forums community India",
        f"{brand} customers love positive reviews",
        f"{brand} complaints problems negative reviews",
        f"{brand} pricing value for money",
        f"{brand} press coverage Economic Times Forbes",
        f"{brand} marketing strategy campaigns hooks",
        f"{brand} brand positioning tagline USP",
        f"{brand} influencer celebrity ambassador",
        f"{brand} bigbasket blinkit zepto swiggy listing",
        f"{brand} annual report profit loss growth",
        f"{brand} brand story founding mission vision",
    ]
    keys = ["overview","financials","registration","startupIndia","zauba","funding",
            "instagram","facebook","linkedin","twitter","youtube","metaAds","googleAds",
            "semrush","audience","competitors","tam","swot","amazon","flipkart",
            "trustpilot","googleReviews","reddit","quora","forums","positives",
            "negatives","pricing","press","marketing","positioning","influencers",
            "quickcommerce","revenue","brandStory"]
...
{{
  ...
  "sem_traffic": {{
    "monthly_visits":"","top_keywords":["","",""],"backlinks":"","traffic_trend":""
  }},
  "icp": {{
    "icps": [
...
def build_ppt(d, yt_real):
    ...
    for i, r in enumerate(ratings):
        ...
        rv = float(r.get("rating","4").replace("X","4").replace("4.X","4.0"))
...
def generate():
    ...
    try:
        print(f"[1/4] Researching {brand}...")
        raw = deep_research(brand)
...
def home():
    return """
    <h2>Brand PPT Generator API</h2>
    <p>POST /generate with {"brand": "Nike"}</p>
    <p>GET /health to check status</p>
    """
